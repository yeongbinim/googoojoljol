# `Caching`을 이용한 쇼핑몰 랭킹 실시간 조회

쇼핑몰 순위를 실시간으로 조회하는 기능을 DB 조회부터 캐싱 전략까지 단계별 전략의 장점 및 단점을 비교해보자.

## V0. DB 조회

가장 먼저 시도한 방식은 DB에서 직접 조회하는 방식이다. 실시간으로 정확한 데이터를 가져올 수 있다는 장점이 있지만, 트래픽이 늘어날 때 DB 부하가 심해지는 게 단점이다.

아래 결과를 보면 viewCount를 기준으로 상위 100개의 쇼핑몰 데이터를 DB에서 조회하는 것을 볼 수 있다. API 시나리오는 다음과 같다.

1. 쇼핑몰 실시간 랭킹 순위에 대해 조회 (Range: 10 ~ 100)
2. `Order By`와 `Limit`를 이용해 상위 100개 데이터 반환

(걸린 시간: `84ms`)

<img src="https://github.com/user-attachments/assets/063ffcca-5172-4197-b908-5b0737bf7d22" width="600" height="300" alt="DB">

## V1.1 `Spring Cache`를 이용한 캐싱 전략

캐싱은 두 가지 방법을 사용했다. 첫 번째로 로컬 캐시인 `Spring Cache`를 사용했고, 다른 하나는 `Redis` 캐싱 방법을 사용했다. 두 방법 모두 다음과 같은 전략으로 동작한다:

1. 랭킹 데이터 조회 시 캐시를 먼저 확인
2. 캐시에 데이터가 없으면 DB에서 조회 후 캐시에 저장
3. 실시간성을 고려해 캐시 `TTL`을 5분으로 설정
...
랭킹 데이터를 `Redis`로 조회한 결과다. 걸린 시간은 `16ms`로 확실히 같은 `Spring-Cache`와 같은 캐싱이지만 네트워크를 타야 하기 때문에 `Spring-Cache (8ms)`보다는 느린 것을 알 수 있다.
...
## V1.3 `Spring-Cache`와 `Redis` 캐싱 방법

후술할 `Redis를 이용한 캐싱 전략 2`를 제외하고 `Spring-Cache`와 `Redis` 캐싱 설정 방법은 크게 다르지 않다. 캐싱을 하는 데 사용되는`CacheManager`를 달리 한 것뿐이다. 아래 코드는 각각의 `CacheManager` 설정 방법이다. 아래 설정에서 볼 수 있는 것처럼, 두 캐싱 방법은 TTL 설정에서 차이를 보인다.`Redis`는 `RedisTemplate`을 이용한 캐싱 방법이 아닌, `CacheManager`를 통해 캐싱하는 경우 `CacheManager`에서 캐싱 데이터 TTL을 설정할 수 있다. `Spring-Cache`의 경우에는 애플리케이션 메모리에 존재하기 때문에 애플리케이션이 종료되면 캐싱 데이터가 사라지므로 별도의 TTL 설정은 불가하다.
...
### `@Cachable`과 `@CachePut`을 조회 어뷰징 관리
...
### 데이터 구조 이중화: `Sorted Set`과 `Hash`의 조합
...
1. <img width="1000" alt="Image" src="https://github.com/user-attachments/assets/7c5a2f7c-af42-4998-853c-5155767e5479" />
...
다행히도 Redis는 이러한 동시성 문제를 해결하기 위한 몇 가지 강력한 기능을 제공한다. 특히, Redis는 싱글스레드로 동작하기 때문에 한 번에 하나의 명령만 실행된다. 덕분에 INCR 같은 연산은 원자적(Atomic) 으로 실행되며, 별도의 락이나 트랜잭션 없이도 동시성 문제를 안전하게 처리할 수 있다.

\+ <u>원자성이란?</u>

원자성(Atomicity)이란 작업이 문자 그대로 더이상 나눌 수 업는 단위로 실행되는 성질을 의미한다. 즉, 작업이 실행되면 전부 실행되거나, 하나도 실행되지 않는다. 이를 통해 중간에 변경 사항이 손실되거나, 연산이 일부만 적용되는 문제를 해결할 수 있다. DB에 데이터를 저장하거나 변경을 가할 때 비즈니스 로직을 'Transaction'이라는 하나의 단위로 묶는 것 역시 연산의 원자성을 지키기 위함이다.
...
### 랭킹 리스트에 없는 쇼핑몰이 조회되면 어떻게 하지?

### 10분마다 DB에 반영하는 건 좋다... 그런데 어떻게 쿼리를 날려야 할까?
